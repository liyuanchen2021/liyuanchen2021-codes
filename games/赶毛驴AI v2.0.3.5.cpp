...
int main() {
	...
	while (1) {
		...
		if (first) {
			...
			cout << "6.先出完者为胜；越先出完牌，拿到的分数越多，每局最高198分；游戏持续到只剩下一位玩家为止" << endl;
			...
			if (F) {
				cout << "欢迎回来，";
				name(p[pos].name + '(' + num_to_str(p[pos].score) + ')');
				cout << '\n';
			} else {
		...
		name2(pc2); cout << endl;
		putline(); setup();
		int hs = 0, xs = rand() % 3, p0c, p1c, p2c, rp0 = 7, rp1 = 7, rp2 = 7, pass = 0, Pass = 0, fe0 = 0, fe1 = 0, fe2 = 0, final, w1, w2, remain = 3, num = 0;
