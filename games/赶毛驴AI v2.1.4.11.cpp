...
int main() {
	...
	while (1) {
		...
		if (first) {
			...
			if (F) {
				cout << "欢迎回来，";
				name(p[pos].name + "(Lv." + num_to_str(get_level(p[pos].score)) + ')');
				cout << '\n';
			} else {
			...
			s = p[pos].name + "(Lv." + num_to_str(get_level(p[pos].score)) + ')';
			cout << "3秒后将进入...\n";
		...
		while ((rp0 != 0) + (rp1 != 0) + (rp2 != 0) > 1) {
			...
			if ((rp0 == 0 || rp1 == 0 || rp2 == 0) && !Flag) {
				...
				if (rp0 == 0) {
					...
					y = rand() % 9 + 1;
					add = fe0 = x * y * 2;
					s = p[pos].name + "(Lv." + num_to_str(get_level(p[pos].score + add)) + ')';
					name(s);
					cout << "获得" << fe0 << "分" << endl;
					Add(fe0);
					fff[0] = true;
		...
		if (final == 0) {
			fe0 = add / 2;
			s = p[pos].name + "(Lv." + num_to_str(get_level(p[pos].score + fe0)) + ')';
			name(s);
			cout << "获得" << fe0 << "分" << endl;
			Add(fe0);
			p0c = -2; 
